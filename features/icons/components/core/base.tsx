...
export function IconBase({ children, title, size = ICON_DEFAULTS.SIZE, className, ...props }: IconProps) {
    const accessible = props["aria-label"] ?? title;
    const isAccessible = Boolean(accessible);
    ...
    return (
        <svg
            ...
            aria-label={isAccessible ? accessible : undefined}
            aria-hidden={isAccessible ? undefined : (props["aria-hidden"] ?? true)}
